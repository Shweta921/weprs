...
$check=mysql_num_rows(mysql_query("select * from `dispatchtemp2` WHERE `PCode`='$PCode'"));
if($check!=0)
		{
	echo"<script>
window.alert('Data Already added');
window.location.href='edit_dispatch.php?id=1';
</script>";
	}
		else
		{
$tempavl=0;
$temptaken=0;
	$qrysch3=mysql_query("select * from dispatchtemp2 WHERE `PCode`='$PCode' AND `po`='$pochaln'");
	 while ($tempdata2 = mysql_fetch_array($qrysch3))
		 {
		$temptaken=$temptaken+$tempdata2['quantity'];	  
		 }

	$qrysch2=mysql_query("select * from dispatch WHERE `PCode`='$PCode' AND `po`='$pochaln'");
...
$PName=$Pdata['PName'];
			...
			$sql="INSERT INTO `dispatchtemp2`
(`challan`,`po`,`date`,`date2`,`PCode`,`spoffice`,`quantity`,`login`) VALUES 
('$challan', '$pochaln', '$podate','$ADate','$PCode','$spoffice','$iqty','$login')";
			$res=mysql_query($sql) or die(mysql_error());
			$sq2="UPDATE `dispatchtemp2` SET `date2`='$ADate',`spoffice`='$spoffice' WHERE `login`='$login' AND `challan`='$challan'";
